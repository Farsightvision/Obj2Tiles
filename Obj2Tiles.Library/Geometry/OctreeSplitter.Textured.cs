...
/// <summary>UV+material-aware face for the textured clipping path.</summary>
...
/// <summary>Textured analogue of <see cref="ClipResult"/>: positions, UVs, and faces.</summary>
...
public static partial class OctreeSplitter
{
    ...
    private static (ClipResultT, ClipResultT) ClipAtAxisT(
        ...
        double split)
    {
        // Snap verts within a scale-relative tolerance onto the clip plane, so
        // adjacent cells agree on boundary verts despite ULP-level plane drift.
        ...
        // Positions dedupe by edge, but UVs key on edge+UV pair: triangles sharing
        // a 3D edge may carry different UVs there, so deduping by edge alone would
        // collapse the seam.
        ...
        // The vertex orderings below preserve CCW for CCW source but flip CW source
        // to back-facing; record source normals so FixWindingT can correct them.
        ...
        // Flip any sub-triangle whose normal disagrees with its source's.
    ...
    /// <summary>Textured recursive split that threads UVs and material indices through every clip.</summary>
    ...
    /// <summary>
    /// Partition a textured mesh into cells at exactly <paramref name="depth"/>;
    /// always reaches the requested depth, ignoring vertex-count budgets.
    /// </summary>
