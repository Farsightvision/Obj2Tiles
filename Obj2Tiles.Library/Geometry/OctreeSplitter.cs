...
/// <summary>
/// Result of clipping a mesh at a plane; boundary vertices have identical
/// positions across both outputs (epsilon = 0).
/// </summary>
...
/// cell-boundary planes so adjacent leaves share exactly equal boundary vertex
/// positions (so the locked border has no cracks).
/// </summary>
public static partial class OctreeSplitter
{
        ...
        IReadOnlyList<Vertex3> verts, IReadOnlyList<Face> faces, int axis, double split)
    {
        ...
        // Per crossing edge, both sides get the same boundary vertex appended; keyed by sorted (a,b).
        ...
        (int li, int ri) GetBoundary(int a, int b)
        {
            ...
            var newV = new Vertex3(
                ...
                va.Y + t * (vb.Y - va.Y),
                va.Z + t * (vb.Z - va.Z));
            // Snap the split axis to the exact plane to avoid FP drift between sides.
        ...
        foreach (var f in faces)
        {
            ...
            {
                // One or more vertices lie on the plane.
                ...
                // onCount == 3: all on plane, degenerate — skip
            }
        }
...
/// <summary>
/// A leaf cell. <see cref="CellBounds"/> is the tight triangle bounds of the
/// leaf's mesh, not the looser cell AABB.
/// </summary>
...
{
    /// <summary>
    /// Slack over maxVertsPerTile (accept a leaf at &lt;= maxVertsPerTile ×
    /// HeadroomFactor) to bound recursion when triangles cluster on a split plane.
    /// </summary>
...
{
    /// <summary>
    /// Recursively split until each leaf is small enough or maxDepth is reached.
    /// Cells with zero faces are pruned (no LeafTile emitted).
    /// </summary>
        ...
        List<LeafTile> leaves)
    {
        ...
        {
            // Split Y only; Z is kept as a full-height column.
...
{
    /// <summary>
    /// 12-element 3D Tiles box [cx,cy,cz, hx,0,0, 0,hy,0, 0,0,hz] for a vertex set.
    /// </summary>
