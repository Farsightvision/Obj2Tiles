...
/// <summary>
/// Pins ConformalHierarchyStage.PredictAtlasSide — it sizes every tile's atlas and feeds the TEXGE-V3
/// meters-per-texel calc, so a regression silently changes texture resolution / VRAM. Contract:
/// side = clamp(NextPow2(round(sqrt(A_world × r_d²))), AtlasMinSize, cap), where cap is MaxAtlasSize for
/// leaves, the per-depth schedule entry if present, else MaxAtlasSizeInternal (fallback MaxAtlasSize).
/// </summary>
public class PredictAtlasSideTests
{
    // Unit square in z=0 → world surface area exactly 1 m².
    ...
    {
        // r_d = 256 (leafDensity at depth==maxDepth); A=1 → idealSide=256 → pow2 256 → within [64,4096].
    ...
    {
        // Same ideal 256, internal node, empty schedule, Internal cap 128 → clamped to 128.
    ...
    {
        // A depth-3 schedule entry (128) wins over a large MaxAtlasSizeInternal (4096) → result 128, not 256.
        var cfg = new AppConfig
        {
    ...
    {
        // Very small tile → idealSide << AtlasMinSize → clamped up to the floor.
