...
public class BoundarySkeletonTests
{
    ...
    public void AddLockAt_multiple_verts_same_depth()
    {
        ...
        skel.AddLockAt(2, 10);
        skel.AddLockAt(2, 11);
        skel.AddLockAt(2, 10); // duplicate, should be deduped
    ...
    {
        var skel = new BoundarySkeleton();
        skel.AddLockAt(1, 5);  // locked at depth 1
        skel.AddLockAt(2, 7);  // locked at depth 2
        var maskAt2 = skel.LockMaskFor(2, vertCount: 10);
        // depth-2 mask must include both depth-1 and depth-2 locks
    ...
    {
        // Source: one triangle (0,0,0), (10,0,0), (5,10,0). Plane: x=5.
        // Triangle straddles. Expected: enriched mesh has new intersection verts on x=5,
        // triangle is split into sub-triangles, skeleton lists those verts at depth 1.
        ...
        // Box has zero Y extent → the Y-plane pass is skipped (extent <= 0 in
        // BuildAndEnrich), leaving only the X plane (x=5) as the active boundary.
    ...
    {
        // Triangle entirely in left half (X<5). Plane x=5 doesn't cross it.
        var verts = new System.Collections.Generic.List<Vertex3>
        {
            new(1, 1, 0),
            new(2, 1, 0),
    ...
    {
        // Source CCW triangle (normal = +Z): (0,0,0), (10,0,0), (5,10,0).
        // After split at x=5, both sub-triangles must keep +Z normal.
        var verts = new System.Collections.Generic.List<Vertex3>
        {
            new(0, 0, 0),
            new(10, 0, 0),
        ...
        var tex = new System.Collections.Generic.List<Vertex2> { new(0, 0), new(1, 0), new(0.5, 1) };
        var faces = new System.Collections.Generic.List<MeshFace> { new(0, 1, 2, 0, 1, 2, 0) };
        var sceneBounds = new Box3(0, 0, 0, 10, 0, 0); // zero-Y to isolate x=5 plane
        ...
        foreach (var f in eFaces)
        {
            ...
            // 2D cross product of (pb-pa) and (pc-pa), z-component:
    ...
    {
        // CCW source, no vertex on plane; one vert left, two right → L1R2.
        // A=(0,0,0) left, B=(8,0,0) right, C=(7,10,0) right. Plane x=5.
        var verts = new System.Collections.Generic.List<Vertex3>
        {
            new(0, 0, 0),
            new(8, 0, 0),
    ...
    {
        // CCW source, two verts left, one right → L2R1.
        // A=(0,0,0) left, B=(2,0,0) left, C=(7,10,0) right. Plane x=5.
        var verts = new System.Collections.Generic.List<Vertex3>
        {
            new(0, 0, 0),
            new(2, 0, 0),
    ...
    {
        // Triangle straddles x=5 (depth-1 plane) AND x=2.5 (depth-2 plane within left half).
        // After maxDepth=2, skeleton must have locks at depths 1 and 2;
        // LockMaskFor(2) must include both sets.
        // A=(0,0,0), B=(10,0,0), C=(5,10,0). Plane d=1: x=5. Planes d=2: x=2.5, x=7.5.
        var verts = new System.Collections.Generic.List<Vertex3>
        {
            new(0, 0, 0),
            new(10, 0, 0),
        ...
        var tex = new System.Collections.Generic.List<Vertex2> { new(0, 0), new(1, 0), new(0.5, 1) };
        var faces = new System.Collections.Generic.List<MeshFace> { new(0, 1, 2, 0, 1, 2, 0) };
        var sceneBounds = new Box3(0, 0, 0, 10, 0, 0); // zero-Y, only X axis subdivides
