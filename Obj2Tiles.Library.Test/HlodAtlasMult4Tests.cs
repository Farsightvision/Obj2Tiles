...
/// <summary>
/// KHR_texture_basisu requires KTX2 width/height to be multiples of 4.
/// s3tc-only WebGL clients (Windows/ANGLE-D3D11: no etc1/astc) reject a
/// non-multiple-of-4 compressedTexImage2D with INVALID_OPERATION; Cesium
/// swallows the GL error, leaving an incomplete texture that samples opaque
/// black — whole tiles render pure black (the HLOD "black tiles" bug).
/// The HLOD atlas save path must therefore never emit an atlas whose edge
...
public class HlodAtlasMult4Tests
{
    ...
    /// <summary>
    /// Drives the real HLOD atlas pipeline (PrepareRepackTextures → FillAtlases
    /// → SaveAtlasesAndUpdateMaterial) over a textured quad whose source texture
    /// size forces the natural-below-cap path to land on a non-multiple-of-4
    /// pack edge, then asserts the SAVED atlas dims are multiples of 4.
    /// </summary>
    [TestCase(70)]
    [TestCase(1914)]
