...
public class ConformalHierarchyStageTests
{
    ...
    {
        // 4 verts (square split into 2 triangles), lock vert 0.
        // faces.Count=2 < 32 triggers early exit — all faces are returned as-is.
        var verts = new[]
        {
            new Vertex3(0, 0, 0),
            new Vertex3(1, 0, 0),
        ...
        // Early exit returns all input faces unchanged.
        bool seen0 = false;
    ...
    {
        // Input: a simple flat 4x4 quad mesh (32 triangles).
        // Build tree with maxDepth=2 → 4 depth-1 cells, 16 depth-2 cells.
        // Verify structure AND adjacent depth-1 cells share boundary verts exactly.
        ...
        // Root tile content should still represent the full mesh.
        Assert.That(root.TileContentT!.Faces.Length, Is.GreaterThanOrEqualTo(32),
            "root must have at least the source triangle count");

        // Invariant: adjacent depth-1 cells (1,0,0,0) and (1,1,0,0)
        // must have BIT-IDENTICAL boundary verts at the shared X plane.
    ...
    {
        // 3D Tiles spec invariant (enforced by ExpandBoundsBottomUp): a node's published
        // bounding volume MUST contain every descendant's. A viewer that culls a parent
        // also culls its whole subtree, so a parent AABB not enclosing its children would
        // pop / drop sub-tiles mid-transition. Per-depth simplification removes different
        // verts at each depth, so a content AABB alone does NOT satisfy this — the bottom-up
        // expansion is load-bearing. This pins it end-to-end on the built tree (previously
        // only structure + conformal boundary-vert sharing were asserted).
        var (verts, tex, faces) = MakeFlatQuadGrid(rows: 4, cols: 4);
        var bounds = ComputeBounds(verts);
    ...
    {
        // Invariant: output MeshFace.MaterialIndex must match the cluster of
        // all three (TexA, TexB, TexC) UVs. A face that claims material X but
        // carries UVs from material Y's cluster would blow up Y's atlas rect
        // at FillAtlases time (historically the source of a 32 GB OOM).
        var (verts, tex, faces) = MakeFlatQuadGridMultiCluster(rows: 8, cols: 8);
        var lockMask = new byte[verts.Count]; // no externally-locked verts
        ...
        // Build per-tex-vert → material map from input. A consistent face must
        // have all 3 tex indices belonging to the same material as the face.
    ...
    {
        // Invariant: positions touching ≥2 distinct materials must be locked
        // from collapse (regardless of supplied lockMask). After simplification
        // at aggressive ratios, every seam position must still appear in the
        // output face list.
        var (verts, tex, faces) = MakeFlatQuadGridMultiCluster(rows: 8, cols: 8);
        var lockMask = new byte[verts.Count]; // no externally-locked verts

        // Identify seam positions (touching ≥2 materials in input).
    ...
    // 4-quadrant multi-cluster grid: each quadrant is one material/cluster,
    // with its own UV space [0..1]^2. Seam verts run along the centerline rows
    // and columns. Produces (rows+1)*(cols+1) positions, ~rows*cols faces.
    private static (List<Vertex3>, List<Vertex2>, List<MeshFace>) MakeFlatQuadGridMultiCluster(int rows, int cols)
    {
        ...
        // For each (pos, material) we need a distinct UV. We'll use a dict
        // (posIndex, materialIndex) → texIndex and append UVs lazily.
        ...
        int MatFor(int r, int c)
        {
            ...
            return top * 2 + right; // 0..3
        }
        // UV space per cluster: each occupies [0..1]^2, scaled to its quadrant size.
        // A vert at (c, r) appearing in cluster M gets UV ((c - cBase)/halfC, (r - rBase)/halfR).
        // cBase/rBase are the cluster's origin corner.
